...
@implementation NotificationDietClass

@synthesize stringMeal, stringTime, database, results, stringNotificationIndex, tempCompliance, tempTotalCount, uuidString;


static NotificationDietClass *singletonInstance;
...
-(void)openNotificationTable{
    ...
    [database executeUpdateWithFormat:@"create table if not exists NotificationDietRecord (indexID integer PRIMARY KEY AUTOINCREMENT NOT NULL, mealType char(30) NOT NULL, notificationFrequency char(15) DEFAULT('Daily'), notificationTime char(25) NOT NULL, notificationCreationDate float(30), notificationUpdateDate float(10), uuid char(50), skipCount float(10) DEFAULT(0));"];
}

-(NSArray *)getAllDietNotificationsFromDatabase{
    
    [self openNotificationTable];
    results = [database executeQueryWithFormat:@"SELECT indexID, mealType, notificationTime FROM NotificationDietRecord"];
    ...
    while([results next])
    {
        ...
        [returnDictionary setObject:@"Diet" forKey:@"NotfiType"];
...
-(void)addNotificationToDatabase:(NotificationDietClass *)reminder{
    ...
    [self openNotificationTable];
    [database executeUpdateWithFormat:@"INSERT INTO NotificationDietRecord (mealType, notificationTime, notificationCreationDate, notificationUpdateDate, uuid) VALUES (%@, %@, %@, %@, %@)",reminder.stringMeal, reminder.stringTime, [NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]], [NSNumber numberWithInt:0], reminder.uuidString];
    ...
    if ([reminder.stringMeal isEqualToString:[LocalizationManager getStringFromStrId:@"Snack"]]) {
        self.tempMealType = 0;
    }else if ([reminder.stringMeal isEqualToString:[LocalizationManager getStringFromStrId:@"Breakfast"]]){
        self.tempMealType = 1;
    }else if ([reminder.stringMeal isEqualToString:[LocalizationManager getStringFromStrId:@"Lunch"]]){
        self.tempMealType = 2;
    }else if ([reminder.stringMeal isEqualToString:[LocalizationManager getStringFromStrId:@"Dinner"]]){
        self.tempMealType = 3;
    }
    ...
    dispatch_promise(^{
        ...
        record.mealType = [NSNumber numberWithInt:self.tempMealType];
...
-(void)updateNotificationToDatabase:(NotificationDietClass *)reminder{

    [self openNotificationTable];
    
    [database executeUpdateWithFormat:@"UPDATE NotificationDietRecord SET mealType=%@, notificationTime=%@, notificationUpdateDate=%@ WHERE indexId=%ld", reminder.stringMeal, reminder.stringTime, [NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]], (long)[reminder.stringNotificationIndex integerValue]];
    [database close];
...
-(void)deleteNotificationFromDatabase:(NSString *)stringID{
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"10.0") ) {
        ...
        results = [database executeQueryWithFormat:@"SELECT uuid FROM NotificationDietRecord WHERE indexId=%ld", (long)[stringID integerValue]];
        ...
        [database executeUpdateWithFormat:@"DELETE FROM NotificationDietRecord WHERE indexId=%ld", (long)[stringID integerValue]];
        [database executeQuery:@"VACUUM NotificationDietRecord"];
...
-(void)scheduleLocalNotificationWithReminder:(NotificationDietClass *)reminder andWithIndexID:(long long)indexID{
    
    LocalNotificationAssistant *localNotif = [LocalNotificationAssistant getInstance];
    [localNotif askForNotificationPermission];
    
    [localNotif addDietLocalNotificationWithFireDate:[self createDateReminder:reminder.stringTime]
...
-(NSString *)getNotificationDietCompliance{
    ...
    if ([database tableExists:@"NotificationDietRecord"]){
        results = [database executeQueryWithFormat:@"SELECT SUM(skipCount) AS compliance FROM NotificationDietRecord"];
    }else{
       return @"0";
    }
...
-(NSString *)getNotificationDietComplianceRate {
    NSString *skipNumStr = [self getNotificationDietCompliance];
    NSString *rt = @"100";
    ...
    [self openNotificationTable];
    results = [database executeQueryWithFormat:@"SELECT datetime(notificationCreationDate, 'unixepoch'), notificationTime FROM NotificationDietRecord"];
    while([results next])
    {
...
-(NSString *)getNotificationDietTotalCount {
    ...
    if ([database tableExists:@"NotificationDietRecord"]){
        results = [database executeQueryWithFormat:@"SELECT COUNT(mealType) AS count FROM NotificationDietRecord"];
    }else{
        return @"0";
    }
...
-(void)addOneToReminderCountSkipForDietCompliance:(int)reminderID{
    [self openNotificationTable];
        [database executeUpdateWithFormat:@"UPDATE NotificationDietRecord SET skipCount = skipCount + 1 WHERE indexId = %d", reminderID];
    [database close];
};
...
-(NSMutableDictionary *)createDictionaryForReminder:(NSString *)reminderID mealType:(NSString *)mealType uuid:(NSString *)uuid{
    ...
    [reminderDictionary setObject:@"Diet" forKey:@"reminderType"];
...
-(NSString *)createAlertMessage:(NSString *)mealTypeName{
    ...
    NSString *alertMessageString = [NSString stringWithFormat:[LocalizationManager getStringFromStrId:@"Diet Reminder: %@"], mealTypeName];
