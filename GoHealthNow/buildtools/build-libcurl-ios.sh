...
make distclean
./configure --disable-shared --enable-ipv6 --enable-static --host="${ARCH}-apple-darwin" --enable-threaded-resolver --with-darwinssl
...
export LDFLAGS="-arch ${ARCH} -isysroot /Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs/iPhoneOS.sdk"
make distclean
./configure --disable-shared  --enable-ipv6 --enable-static --host="aarch64-apple-darwin" --enable-threaded-resolver --with-darwinssl
make -j `sysctl -n hw.logicalcpu_max`
cp lib/.libs/libcurl.a "${HOME}/Desktop/libcurl-${ARCH}.a"
