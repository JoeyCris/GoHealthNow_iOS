...
@interface BloodGlucoseNotificationViewController () <UITextFieldDelegate>
...
@implementation BloodGlucoseNotificationViewController
...
-(void)setupMealLabel{
    ...
    self.lblMealType.layer.borderColor = [UIColor colorWithRed:53/255.0f green:86/255.0f blue:164/255.0f alpha:1].CGColor;
    self.lblMealType.layer.borderWidth = 1.0;
    self.lblMealType.textColor = [UIColor colorWithRed:53/255.0f green:86/255.0f blue:164/255.0f alpha:1];
    self.lblMealType.font = [UIFont systemFontOfSize:20];
    
    [self setupMealDisplay];    
}
...
-(void)setupMealDisplay{
    ...
    if ([components hour]>=5 && [components hour] < 8){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEFORE_BREAKFAST];
    }
    else if ([components hour]>=8 && [components hour] < 10){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_AFTER_BREAKFAST];
    }
    
    else if ([components hour]>=10 && [components hour] < 12){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEFORE_LUNCH];
    }
    else if ([components hour]>=12 && [components hour] < 16){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_AFTER_LUNCH];
    }
    
    else if ([components hour]>=16 && [components hour] < 18){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEFORE_DINNER];
    }
    else if ([components hour]>=18 && [components hour] < 21){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_AFTER_DINNER];
    }
    else if ([components hour]>=21 && [components hour] < 5){
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEDTIME];
    }
    
    else {
        self.lblMealType.text = [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_OTHER];
    }
}
...
-(void)configurePickerViewer{
    ...
    self.arrayMeals = [[NSMutableArray alloc] initWithObjects:[LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEFORE_BREAKFAST],
         [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_AFTER_BREAKFAST],
            [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEFORE_LUNCH],
             [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_AFTER_LUNCH],
           [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEFORE_DINNER],
            [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_AFTER_DINNER],
                 [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_BEDTIME],
                   [LocalizationManager getStringFromStrId:MSG_ADD_GLUCOSE_RECORD_MEAL_TYPE_OTHER],
                       nil];
...
- (void)btnDone {
    if ([[NotificationDuplicateCheckClass getInstance] isDuplicateNotificationTimeUsingNotificationTimeString:self.lblTime.text]) {
        ...
        int mealIndex = (int)[self.arrayMeals indexOfObject:self.lblMealType.text];
        
        
        NotificationBloodGlucoseClass *reminder = [NotificationBloodGlucoseClass getInstance];
        [reminder addReminderMeal:self.lblMealType.text];
        [reminder addReminderTime:self.lblTime.text];
        reminder.uuidString = [[NSUUID UUID] UUIDString];
        
        [reminder addNotificationToDatabase:reminder andIndexOfMealType:mealIndex];
