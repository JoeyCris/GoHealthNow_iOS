...
- (void)startCaptureSession {
    ...
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Starting Capture Session...");
    ...
    // Log video check
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Checking for video input...");
    
    if (videoInput) {
        // Log video input
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Valid video input");
        ...
        // Log metadata setup
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Setting up metadata output...");
        
        // Start the metadata output
        [self setupMetadataOutput];
        
        // Log preview layer creation
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Creating the preview layer...");
        ...
        // Log capture session
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Started Capture Session");
        
        // Start Scan
        [self startScanSession];
        
    } else {
        NSLog(@"[BarcodeScannerView] Invalid video input. There is no video input, or the scanner was not able to obtain input.");
        NSLog(@"[BarcodeScannerView] %@", error);
...
- (void)stopCaptureSession {
    ...
    // Log the stop
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Stopped capture session");
}

- (void)startScanSession {
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Starting Scan Session...");
    
    if (([self isCaptureSessionInProgress] == YES) && ([self isScanSessionInProgress] == NO)) {
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Capture session is in progress, but not a scan session. Begginning scan session...");
        
        // Add the metadata output to the session - there is a running capture session, but no scan session
        [self setupMetadataOutput];
        
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Scan session started");
    } else if ([self isCaptureSessionInProgress] == NO) {
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Capture session not in progress, starting new session");
    ...
    if (animateScanner) {
        ...
        boundingBox = [[OutlineBox alloc] initWithFrame:self.bounds];
        boundingBox.alpha = 0.0;
...
- (void)stopScanSession {
    ...
    // Log the stop
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Stopped scan session");
}

- (void)setupMetadataOutput {
    // Log the metadata object
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Creating metadata object");
...
- (void)breakdownMetadataOutput {
    // Log the breakdown
    if (verboseLogging) NSLog(@"[BarcodeScannerView] Breaking down metadata output");
...
- (void)setupPreviewLayer {
    ...
    previewLayer.position = CGPointMake(CGRectGetMidX(self.layer.frame), CGRectGetMidY(self.layer.frame));
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
	...
	for (AVMetadataObject *metadataObject in metadataObjects) {
        ...
        // Log the object
        if (verboseLogging) NSLog(@"[BarcodeScannerView] Scanned metadata object: %@", metadataObject);
