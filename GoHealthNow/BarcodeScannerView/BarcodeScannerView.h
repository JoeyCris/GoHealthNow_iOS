...
//
    ...
    #error BarcodeScannerView is built with Objective-C ARC. You must enable ARC for these files.
#endif

#ifndef __IPHONE_7_0
    #error BarcodeScannerView is built with features only available is iOS SDK 7.0 and later.
#endif
...
NS_CLASS_AVAILABLE_IOS(7_0) @interface BarcodeScannerView : UIView <AVCaptureMetadataOutputObjectsDelegate>
...
/// The BarcodeScannerView delegate object used to set the delegate. The delegate reports scan data, errors, and requests information from the delegate.
@property (nonatomic, weak) IBOutlet id <ScannerViewDelegate> delegate;
...
/** Starts the current barcode scanner capture session
 @discussion This method should be called when the encapsulating UIViewController is presented or is loaded, or at any appropriate time. A session will not automatically start when the BarcodeScannerView is loaded (ex. by an interface file). Calling this method begins the AVCaptureSession and starts the collection of camera data - including scan data. */
